...
class RaspiCam(PipelinePart):
    ...
    expmode, width, height, framerate
    """
    def __new__(cls, **kwargs):
        ...
        return super().__new__(cls, (
            'rpicamsrc name={src_name} preview=false vflip={vflip} hflip={hflip} exposure-compensation={ec} '
            + awb_str
            + exp_str +
...
class H264Video(PipelinePart):
    ...
    """
    def __new__(cls, **kwargs):
        defs = merge_defaults(kwargs)
        params_str = ''
        # if defs['h264encoder'] == 'omxh264enc':
        #     params_str = ' quant-i-frames={quant_i_frames}'
        return super().__new__(cls, (
            '{h264encoder}' + params_str
        ).format(**defs))
